...
// 💡 メモリ使用量: 常に30-50MB程度（FastExcel Reader使用）
// 💡 100万行でも問題なし！
```
...
| 項目 | 従来の方法 | ストリーミング処理 |
|------|-----------|------------------|
| **メモリ使用量** | 全件分（数GB） | 常に30-50MB（FastExcel Reader） |
| **処理速度** | 全件読み込み後に処理 | 読み込みながら処理 |
| **大量データ対応** | ❌ OutOfMemoryError | ✅ 100万行でもOK |
| **フリガナ付きセル値** | ❌ 削除が困難 | ✅ 自動削除 |
...
| ✅ forEach処理 | 約30-50MB | 3.0秒 |
| ✅ バッチ処理(100件) | 約30-50MB | 2.8秒 |

**結論: ストリーミング処理でメモリを90%以上削減！FastExcel Readerにより、フリガナ付きセル値も正しく処理できます。**

※ ヘッダー名は前後空白をトリムした上で厳密一致（中間の空白はそのまま）
